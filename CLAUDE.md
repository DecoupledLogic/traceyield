...
- **Windows-first, but multi-machine.** `run.cmd` is now portable (self-locating dir + `PYTHON` override), and artifacts are namespaced per machine under `machines/<machine_id>/` (see "Per-machine data" above). One display wart remains: the `clean()` JS helper still hardcodes the `C--Users-charl-source-repos-` project-dir prefix, so on other machines (whose project dirs have a different prefix) the "Cost by project" labels won't be stripped. It's cosmetic — fix by making `clean()` prefix-agnostic if it matters.
...
- `machines/<id>/{daily_metrics.json, session_metrics.json, report.html, run.log}` and the shared `pricing_history.json` are **generated artifacts** committed alongside the code — regenerating them is expected, not a mistake. A run on a new machine creates a new `machines/<its-hostname>/` folder.
