# TokenOps — the framework and the tool

*Product / vision doc. Written 2026-07-11. This defines **what TokenOps is** — the
discipline — and how **tokenops**, the tool in this repo, implements it. It sits
...
**TokenOps** is a closed-loop discipline for managing the **cost and efficacy of
...
is the *material*. TokenOps runs a loop over that material — **describe → diagnose
→ predict → prescribe → remediate** — and keeps running it, so a team gets steadily
cheaper *and* more effective at using coding agents over time. **tokenops** (this
...
TokenOps is the practice that closes those three gaps for a team: shared
...
more expensive session that avoids three failed reruns is. So TokenOps measures in
...
The operations in TokenOps are five rungs of a single closed loop. Each rung
...
Most usage tools stop at rung 1–2. The `Ops` in TokenOps is the promise that the
...
4. **Provider-blind.** TokenOps is about LLM interactions in general. Claude,
...
Prescriptions in TokenOps pull on a known, growing set of levers — each with a
...
`tokenops` is the reference implementation of the framework in this repo. It is a
...
- **TokenOps** (prose, capitalized) — the framework / the discipline.
- **tokenops** (lowercase) — the tool that implements it (this repo, its package,
  scripts, and identifiers).

Both supersede the earlier working names **TokenLens / Token Lens** and the
...
- [`canonical-data-model.md`](./canonical-data-model.md) — the provider-blind
  record stream that makes TokenOps provider-blind.
